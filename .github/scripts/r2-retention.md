...
targets, and the genuinely separate vendor in the 3-2-1-1-0 design (see the backup feature plan's
"2 distinct media/platforms" row).

## Object Lock

Object Lock is enabled via R2's Bucket Lock feature (available on the free tier — confirmed
2026-08-15), in **Governance** mode (not Compliance): the write-only CI token can never bypass a
...
into `permanent/` is always a manual, hand-run copy (see the backup feature plan's GFS section) —
there is no automated path that could ever populate this prefix incorrectly at scale.
...
scoped tokens, zero egress cost) rather than a cap — see the backup feature plan's open questions
