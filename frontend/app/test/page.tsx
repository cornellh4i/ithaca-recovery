...
const App = () => {
  const sampleMeeting = {
    ...
    description:
      "Discuss project goals, milestones, and next steps with the team.",
  ...
  const [selectedMeetingType, setSelectedMeetingType] =
    useState<string>("Hybrid"); // Default meeting type
  ...
  const [selectedZoomAccount, setSelectedZoomAccount] = useState<string | null>(
    null
  ); // Initially no Zoom account selected
  ...
  const meetingTypeOptions = ["AA", "Al-Anon", "Other"];
  ...
  const createAdmin = async () => {
    try {
      ...
      alert(
        "Admin created successfully! Please check the Admin collection on MongoDB."
      );
    } catch (error) {
  ...
  const getAdmin = async () => {
    try {
      ...
      console.log(adminResponse);
      alert(
        "Admin retrieved successfully! Please check the Admin collection on MongoDB."
      );
    } catch (error) {
  ...
  const deleteAdmin = async () => {
    try {
      ...
      console.log(adminResponse);
      alert(
        "Admin deleted successfully! Please check the Admin collection on MongoDB."
      );
    } catch (error) {
  ...
  const updateAdmin = async () => {
    try {
      /* Configure to be an Admin's real uid */
      const newAdminInfo = {
        uid: "95153",
        name: "Samantha Cruz",
      ...
      console.log(adminResponse);
      alert(
        "Admin updated successfully! Please check the Admin collection on MongoDB."
      );
    } catch (error) {
  ...
  const createMeeting = async () => {
    try {
      ...
      alert(
        "Meeting created successfully! Please check the Meeting collection on MongoDB."
      );
    } catch (error) {
      console.error("There was an error fetching the data:", error);
    }
  ...
  const deleteMeeting = async () => {
    try {
      ...
      console.log(meetingResponse);
      alert(
        "Meeting deleted successfully! Please check the Meeting collection on MongoDB."
      );
    } catch (error) {
      console.error("There was an error fetching the data:", error);
    }
  ...
  const updateMeeting = async () => {
    try {
      ...
      console.log(meetingResponse);
      alert(
        "Meeting updated successfully! Please check the Meeting collection on MongoDB."
      );
    } catch (error) {
      console.error("There was an error fetching the data:", error);
    }
  ...
  const getMeeting = async () => {
    ...
    try {
      const response = await fetch(`/api/retrieve/meeting/${meetingId}`, {
        method: "GET",
      });
      if (!response.ok) {
  ...
  const getMeetingsDay = async () => {
    try {
      ...
      console.log(meetingResponse);
      alert(
        `Daily meetings retrieved successfully for ${currentDate.toISOString()}! Please check the console.`
      );
    } catch (error) {
      console.error("There was an error fetching the data:", error);
    }
  ...
  const getMeetingsWeek = async () => {
    try {
      ...
      console.log(meetingResponse);
      alert(
        `Weekly meetings retrieved successfully for ${currentDate.toISOString()}! Please check the console.`
      );
    } catch (error) {
      console.error("There was an error fetching the data:", error);
    }
  ...
  const getMeetingsMonth = async () => {
    try {
      ...
      console.log(meetingResponse);
      alert(
        `Monthly meetings retrieved successfully for ${currentDate.toISOString()}! Please check the console.`
      );
    } catch (error) {
      console.error("There was an error fetching the data:", error);
    }
  ...
  return (
    <div className={styles["apicontainer"]}>
      ...
      <div className={styles.section}>
        <h2>Admins</h2>
        <TestButton
          testFunc={createAdmin}
          text="Call create admin post /api/write/admin"
        />
        <TestButton
          testFunc={getAdmin}
          text="Call get admin /api/retrieve/admin"
        />
        <TestButton
          testFunc={deleteAdmin}
          text="Call delete admin /api/delete/admin"
        />
        <TestButton
          testFunc={updateAdmin}
          text="Call update admin /api/update/admin"
        />
      </div>
      <div className={styles.section + " " + styles.meetings}>
        <h2>Meetings</h2>
        <TestButton
          testFunc={createMeeting}
          text="Call Create Meeting /api/write/meeting"
        />
        <TestButton
          testFunc={updateMeeting}
          text="Call Update meeting /api/update/meeting"
        />
        <TestButton
          testFunc={deleteMeeting}
          text="Call Delete meeting /api/delete/meeting"
        />
        <TestButton
          testFunc={getMeeting}
          text="Call Get meeting /api/retrieve/[id]"
        />

        <TestButton
          testFunc={getMeetingsDay}
          text="Get Meetings (Day) /api/retrieve/meeting/day"
        />
        <TestButton
          testFunc={getMeetingsWeek}
          text="Get Meetings (Week) /api/retrieve/meeting/week"
        />
        <TestButton
          testFunc={getMeetingsMonth}
          text="Get Meetings (Month) /api/retrieve/meeting/month"
        />
      </div>
      ...
      <div className={styles.section + " " + styles.meetings}>
        ...
        <TestButton
          testFunc={getGroups}
          text="Call get groups /api/groups/routes"
        />
        <TestButton
          testFunc={getCalendars}
          text="Call get calendars /api/calender/getCalendars/routes"
        />
      </div>
      <div className={styles.section}>
      ...
      <div className={styles.section}>
        <h2>Calendar Day Box (Monthly View)</h2>
        <div
          style={{
            width: "630px",
            height: "200px",
        ...
        >
          <BoxText
            ...
            meetings={[
              ...
              { time: "10am", title: "Meeting Name", id: "meet-2", room: "room-1"},
              { time: "8am", title: "Meeting Name", id: "meet-3", room: "room-1" },
              { time: "8pm", title: "Meeting Name", id: "meet-4", room: "room-1" },
            ]}
            onClick={(meetingId) =>
          ...
          <BoxText
            ...
            view="monthly"
            meetings={[
              { time: "12am", title: "Meeting Name", id: "meet-1", room: "Serenity Room" },
              { time: "7am", title: "Meeting Name", id: "meet-2", room: "Seeds of Hope" },
              { time: "12am", title: "Meeting Name", id: "meet-3", room: "Unity Room" },
              { time: "7pm", title: "Meeting Name", id: "meet-4", room: "Serenity Room" },
              { time: "9am", title: "Meeting Name", id: "meet-5", room: "Serenity Room" },
              { time: "9pm", title: "Meeting Name", id: "meet-5", room: "Serenity Room" },
              { time: "12pm", title: "Meeting Name", id: "meet-5", room: "Serenity Room" },
            ]}
            onClick={(meetingId) =>
              console.log(`Meeting clicked: ${meetingId}`)
            }
