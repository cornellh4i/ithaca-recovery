...
const RecurringMeetingForm: React.FC<{ onRecurrenceDataChange: (data: any) => void }> = ({ onRecurrenceDataChange }) => {
  ...
  const handleRecurringChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onRecurrenceDataChange({ isRecurring: e.target.checked });
  };
  ...
  const handleFrequencyChange = (value: number) => {
    setFrequency(value);
    onRecurrenceDataChange({ frequency: value });
  };

  const handleEndOptionChange = (option: string) => {
    setEndOption(option);
    onRecurrenceDataChange({ endOption: option });
