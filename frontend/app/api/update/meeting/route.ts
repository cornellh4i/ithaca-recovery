  ...
  try {
    ...
    if (!meeting) {
      return NextResponse.json(
        { error: "No meeting data provided" },
        { status: 400 }
      );
    }
    ...
    if (!existingMeeting) {
      ...
      return NextResponse.json(
        { error: `Meeting with ID ${mid} not found` },
        { status: 404 }
      );
    }

    const isRecurring =
      !!existingMeeting.recurrencePattern || existingMeeting.isRecurring;
    ...
    if (dataWithoutMid.recurrencePattern) {
      ...
      const { id, numberOfOccurrences, ...patternWithoutId } =
        dataWithoutMid.recurrencePattern;
    ...
    if (isRecurring) {
      if (!recurringOption) {
        return NextResponse.json(
          { error: "Update option is required for recurring meetings" },
          { status: 400 }
        );
      }

      console.log(
        "Handling recurring meeting update with option:",
        recurringOption
      );
      ...
      switch (recurringOption) {
        case "this":
          ...
          if (!currentOccurrenceDate) {
            return NextResponse.json(
              { error: "Current occurrence date is required for this option" },
              { status: 400 }
            );
          }
          ...
          if (existingMeeting.recurrencePattern) {
            try {
              ...
              if (
                ...
                // Update the main recurring meeting
                ...
                // Add the current occurrence to excluded dates
                ...
                // Calculate the start time components (hours, minutes) to preserve meeting time
                ...
                const newEndDateTime = new Date(
                  newStartDateTime.getTime() + meetingDuration
                );

                // Generate a new UUID for the mid field
                const newMid = uuidv4();

                // Create a new non-recurring meeting for this specific occurrence
                const newOccurrence = await prisma.meeting.create({
                  data: {
                    ...
                    isRecurring: false, // One-time meeting
                    recurrencePattern: undefined, // No recurrence
                  },
                });
            ...
            // If it's recurring but has no pattern, update directly
            const updatedMeeting = await prisma.meeting.update({
              where: { mid },
              data: dataWithoutMid,
            });
        ...
        case "thisAndFollowing":
          ...
          if (existingMeeting.recurrencePattern) {
            try {
              ...
              console.error(
                "Error updating this and following occurrences:",
                error
              );
              return NextResponse.json(
                {
