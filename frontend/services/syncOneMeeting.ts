...
import { createCalendarEvent, updateCalendarEvent, reconcileMeetingCalendars } from "./googleCalendar";
import { createZoomMeeting, getZoomHostCapacities, getZoomMeetingCredentials, updateZoomMeeting, getZoomMeetingInvitation, resolveZoomHost, zoomHostPool, zoomRoomCalendarId } from "./zoom";
...
export type SyncOneMeetingResult =
  | { notFound: true }
  | {
      notFound?: false;
...
/**
 * The full idempotent reconcile behind "Retry sync": re-asserts the meeting's Zoom meeting
 * (adopting live credentials, minting or adopting the family's shared meeting when zid-less)
 * and rewrites its calendar events. Shared by the per-meeting retry route
 * (app/api/update/meeting/sync) and the admin bulk title-resync sweep -- both must reconcile
 * exactly the same way or the sweep could leave state a later retry disagrees with.
 * A Suspended meeting is a no-op (its events are deliberately absent), returning the stored
 * statuses unchanged.
 */
export async function syncOneMeeting(mid: string, accessToken: string): Promise<SyncOneMeetingResult> {
    ...
    return { googleSyncStatus, googleSyncError, zoomSyncStatus, zoomSyncError };
}
