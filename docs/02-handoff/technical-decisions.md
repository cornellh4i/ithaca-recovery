...
**A linked-schedule family is a claim too:** a family is served by one Zoom meeting, and `update/meeting/sync`'s retry is where it gets minted. Two retries on the two rows of one un-provisioned family may resolve *different* pool hosts, so the host claims alone never serialize them — the same `lockResourceClaims` call therefore also takes a `zoomFamily` claim keyed on the family's anchor mid, and the reservation that transaction commits is written to every Zoom-bearing member. The Zoom API call still runs after the transaction commits (nothing external ever runs inside one), so the second retry reads that reservation, sees the family is already being provisioned, and defers rather than minting a second meeting.
...
- **A pure Zoom Room change moves in place too (GitHub #522)** — room and host are independent resources (above), so a managed meeting's Zoom Room change never touches the Zoom meeting itself: `zid`/link/passcode/host stay put, and only the join-link event moves — the old room's calendar event is deleted, a fresh one is created on the new room's calendar with the same stored link. Tear-down-and-recreate is reserved for a genuine host-can't-transfer reason (an explicit host change Zoom refused to move, or a shared-`zid` row whose host couldn't move); a room change bundled with one of those still recreates, except on a `zid` another row shares — recreating there would leave the siblings on the old Zoom meeting, so the host move is refused the same way an untransferable shared meeting's is, and only the room move applies. The whole-series edit's synchronous host-resolution step no longer treats a room change as needing a new host either — only "never had a `zid`" or an explicit host change does.
...
- **The second schedule is a bordered card in the recurrence slot, not inline fields.** Its Mode / Days / Room controls are the form's own components, but they render inside `MeetingSchedules.tsx`'s `.scheduleCard` rather than in the positions the meeting's own Mode and Room fields occupy. Chosen over inlining them: the two schedules use the *same* control set, so side-by-side unboxed copies would read as one form with duplicated fields, and there'd be nothing to attach the card's Cancel and Done affordances or the inherited-time note to. It mirrors `RecurringMeeting.module.scss`'s own `.isRecurring` box — a peer of the meeting's schedule, not a nested sub-form. Starting it is a single click: "Add another mode for other days" sits under the open recurrence editor and both collapses that editor into its summary card and opens the new card. The trigger appears only once the meeting's own schedule is a weekly one with at least one weekday and a readable time — the schedule the card summarises and the linked row inherits. Done then collapses the composed card into a summary of its own (with an Edit link back), so the two schedules read alike; it is display state only, gated on the same completeness the summary needs and never on what gets submitted. Editing the meeting off Weekly discards the draft and leaves a notice in its place, since a linked schedule has no representation outside a weekly union.
