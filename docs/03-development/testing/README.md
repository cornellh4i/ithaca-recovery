...
forever. There's no such file in the suite currently; when a manual-script feature needs this
treatment, create `tests/e2e/provisional.spec.ts` with a `describe("provisional — unimplemented
features")` block and one tagged test per stubbed feature. See
[Technical Decisions](../../02-handoff/technical-decisions.md#testing-strategy-playwright-primary-jest-for-narrower-jobs)
for background on the pattern.
